...
class AnalysisConfigurationsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should get detail page' do
    ...
    assert_select 'table#analysis-parameters', 1, 'Did not find analysis parameters table'
    assert_select 'tr.analysis-parameter-entry', 3, 'Did not find correct number of analysis parameter entries'
    puts "#{File.basename(__FILE__)}: #{self.method_name} successful!"
  end
