        ...
        begin
          ...
          if remote_file.present?
            ...
            if remote_file.generation.to_s == study_file.generation
              Rails.logger.info "#{Time.now}: generation tags for #{study_file.bucket_location}:#{study_file.id} match, performing cleanup"
            else
              Rails.logger.info "#{Time.now}: generation tags for #{study_file.bucket_location}:#{study_file.id} do not match, updating database records"
              study_file.update(generation: remote_file.generation)
              Rails.logger.info "#{Time.now}: generation tag for #{study_file.bucket_location}:#{study_file.id} updated, performing cleanup"
            end
            # once everything is in sync, perform cleanup
            study_file.remove_local_copy
            Rails.logger.info "#{Time.now}: cleanup for #{study_file.bucket_location}:#{study_file.id} complete"
          else
