class FireCloudClient < Struct.new(:access_token, :api_root, :storage, :expires_at)
	...
	# location of Google service account JSON (must be absolute path to file) //This is being hardcoded in order to work, Must be reverted for actual productionZ
	#SERVICE_ACCOUNT_KEY = File.absolute_path('config/broad-singlecellportal.json')
