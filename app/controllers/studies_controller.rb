class StudiesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @study.save
        ...
        format.html { redirect_to merge_default_redirect_params(path, branding_group: params[:branding_group]),
  ...
  def sync_study
    ...
    begin
      ...
      redirect_to merge_default_redirect_params(studies_path, branding_group: params[:branding_group]), alert: "We were unable to sync with your workspace bucket due to an error: #{view_context.simple_format(e.message)}" and return
    end
  ...
  def sync_submission_outputs
    ...
    begin
      ...
      redirect_to merge_default_redirect_params(request.referrer, branding_group: params[:branding_group]), alert: "We were unable to sync the outputs from submission #{params[:submission_id]} due to the following error: #{e.message}"
    end
  end
  ...
  def update
    ...
    respond_to do |format|
      if @study.update(study_params)
        ...
        format.html { redirect_to merge_default_redirect_params(studies_path, branding_group: params[:branding_group]), notice: "Study '#{@study.name}' was successfully updated." }
        format.json { render :show, status: :ok, location: @study }
      else
    ...
    if @study.can_delete?(current_user)
      ...
      if params[:workspace] == 'persist'
        ...
        begin
          ...
          redirect_to merge_default_redirect_params(studies_path, branding_group: params[:branding_group]), alert: "We were unable to delete your study due to: #{view_context.simple_format(e.message)}.<br /><br />No files or database records have been deleted.  Please try again later" and return
        end
      end
      ...
      respond_to do |format|
        format.html { redirect_to merge_default_redirect_params(studies_path, branding_group: params[:branding_group]), notice: update_message }
        format.json { head :no_content }
      end
    else
      redirect_to merge_default_redirect_params(studies_path, branding_group: params[:branding_group]), alert: 'You do not have permission to perform that action' and return
    end
  end
  ...
  def check_edit_permissions
    if !user_signed_in? || !@study.can_edit?(current_user)
      ...
      respond_to do |format|
        ...
        format.html {redirect_to merge_default_redirect_params(studies_path, branding_group: params[:branding_group]),
