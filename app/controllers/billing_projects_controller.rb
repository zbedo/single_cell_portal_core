class BillingProjectsController < ApplicationController
  ...
  def create
    ...
    begin
      ...
      redirect_to merge_default_redirect_params(billing_projects_path, brand: params[:brand]), notice: "Your new project '#{project_name}' was successfully created using '#{billing_account}'" and return
    rescue => e
      logger.error "#{Time.now}: Unable to create new billing project #{project_name} due to error: #{e.message}"
      redirect_to merge_default_redirect_params(billing_projects_path, brand: params[:brand]), alert: "We were unable to create your new project due to the following error: #{e.message}" and return
    end
  end
  ...
  def create_user
    ...
    begin
      ...
      redirect_to merge_default_redirect_params(billing_projects_path, brand: params[:brand]), notice: "#{email} has successfully been added to #{params[:project_name]} as #{role}" and return
    rescue => e
      logger.error "#{Time.now}: Unable to add #{email} to #{params[:project_name]} due to error: #{e.message}"
      redirect_to merge_default_redirect_params(new_billing_project_user_path, brand: params[:brand]), alert: "We were unable to add #{email} to #{params[:project_name]} due to the following error: #{e.message}" and return
    end
  end
  ...
  def delete_user
    ...
    begin
      ...
      redirect_to merge_default_redirect_params(billing_projects_path, brand: params[:brand]), notice: "#{email} has successfully been removed from #{params[:project_name]} as #{role}" and return
    rescue => e
      logger.error "#{Time.now}: Unable to remove #{email} from #{params[:project_name]} due to error: #{e.message}"
      redirect_to merge_default_redirect_params(billing_projects_path, brand: params[:brand]), alert: "We were unable to remove #{email} from #{params[:project_name]} due to the following error: #{e.message}'" and return
    end
  end
  ...
  def check_project_permissions
    ...
    unless projects.map {|project| project['projectName']}.include?(params[:project_name])
      redirect_to merge_default_redirect_params(billing_projects_path, brand: params[:brand]), alert: 'You do not have permission to perform that action.' and return
    end
  end
end
