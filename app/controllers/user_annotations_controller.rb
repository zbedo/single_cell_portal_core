class UserAnnotationsController < ApplicationController
  ...
  def persist_to_study
    respond_to do |format|
      ...
      format.html { redirect_to user_annotations_path, notice: "User Annotation '#{@user_annotation.name}' will be added to the study. You will receive an email upon completion or error." }
      format.json { render :index, status: :ok, location: user_annotations_path }
      @user_annotation.persist_to_study(current_user)
    end
  end
