class UserAnnotationsController < ApplicationController
  ...
  def persist_to_study
    ...
    annot_name = @user_annotation.name
    ...
    user_annotation_array = @user_annotation.concatenate_user_data_arrays(annot_name,'annotations')

    # create new annotation data arrays and add them
    user_annotation_data_arrays = @user_annotation.user_data_arrays.where(array_type: 'annotations').to_a
    x_arrays = @user_annotation.user_data_arrays.where(array_type: 'coordinates', name: 'x').to_a
    y_arrays = @user_annotation.user_data_arrays.where(array_type: 'coordinates', name: 'y').to_a
    text_arrays = @user_annotation.user_data_arrays.where(array_type: 'cells').to_a
    ...
    annot_hash = {'name'=>annot_name, 'type'=>'group','values'=>@user_annotation.values, 'header_index'=>(types.length-1)}
    ...
    respond_to do |format|
      if cluster.update(cell_annotations: cluster_annotations)
        ...
        #If successful, redirect back and say success
        format.html { redirect_to user_annotations_path, notice: "User Annotation '#{annot_name}' was successfully updated." }
        format.json { render :index, status: :ok, location: user_annotations_path }
      else
        #If an error, show it
