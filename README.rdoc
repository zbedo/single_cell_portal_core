...
* <b>GCP Service Account keys</b>: Regardless of where the portal is deployed, it requires a Google Cloud Platform Service Account in order to make authenticated calls into FireCloud and Google Cloud Storage.  Therefore, you must create a Google Cloud Platform project and export the default service account key.  See https://developers.google.com/identity/protocols/OAuth2ServiceAccount for more information about service accounts, or https://console.developers.google.com/permissions/serviceaccounts to create and export keys (exported keys must be in JSON format).
