...
Once the image has successfully built and the database container is running, use the following command to start the container:
  bin/boot_docker -u (sendgrid username) -P (sendgrid password)
...
This sets up several environment variables in your shell and then runs the following command:
  docker run --rm -it --name $CONTAINER_NAME -p 80:80 -p 443:443 -p 587:587 --link mongodb:mongodb -h localhost -v $PROJECT_DIR:/home/app/webapp:rw -e PASSENGER_APP_ENV=$PASSENGER_APP_ENV -e MONGO_LOCALHOST=$MONGO_LOCALHOST -e SENDGRID_USERNAME=$SENDGRID_USERNAME -e SENDGRID_PASSWORD=$SENDGRID_PASSWORD -e SECRET_KEY_BASE=$SECRET_KEY_BASE single_cell_docker
...
* MongoDB WiredTiger cache size: 10GB (always enabled)
...
studies in the portal for QA purposes, as well as receive certain admin-related emails (see PRODUCTION PERSISTENT DISKS
below[#user-content-production-persistent-disks]).  This can only be enabled manually through the console.

To create an admin user account:
1. Create a user account normally through the UI (using sign-up or Google auth features)
2. Start the portal locally (or ssh into production VM)
3. Connect to the running portal container: <code>docker exec -it single_cell bash</code>
4. Query for the desired user account: <code>user = User.find_by(email: '<email address here>')</code>
5. Set the admin flag to true: <code>user.update(admin: true)</code>
...
==== PRODUCTION PERSISTENT DISKS
The production Single Cell portal utilizes two persistent disks for data storage: one for the portal itself, and a second
non-boot persistent disk for data (mounted at <code>/home/docker-user/single_cell_portal/data</code>).  During normal
operation and updates, there is nothing that needs to be done pertaining these disks.  When launching the portal in
production mode, the <code>bin/boot_docker</code> script automatically enables a second mount at the
<code>/home/app/webapp/data</code> location that reads from this secondary data disk.  <em>Note: when running locally in
production mode, this second mount has no effect, but in Google it allows Docker to read from the second partition since
it cannot distinguish the additional disk via the mountpoint).</em>

There is a weekly cron that calls a script (bin/weekly_disk_snapshot.bash) that backs up both disks and removes any
snapshots over a month old.  Also, a daily cron runs that calls a Rails mailer method <code>(SingleCellMailer.daily_disk_status)</code>
that will send a daily email to portal admins.  However, if the host VM is ever stopped or restarted (if the VM is
upgraded, or a disk is resized), then the data disk must be manually remounted.  If this is not done, data uploads will
continue to deposit on the boot disk, which has a limit of 2TB and could run out of space, causing the application to
die unrecoverably.

To manually remount the data volume:
1. SSH into production VM
2. Find data disk volume name: <code>ls /dev/disk/by-id</code> (the name should be google-singlecell-production-data)
3. Verify that a data directory exists, and create if not: <code>mkdir /home/docker-user/single_cell_portal/data</code>
4. Remount the disk:
	sudo mount -o discard,defaults /dev/disk/by-id/google-singlecell-production-data /home/docker-user/single_cell_portal/data
5. If a disk has been resized in the Google Console, extend the filesystem:
	sudo resize2fs /dev/disk/by-id/google-google-singlecell-production-data

More information is available at https://cloud.google.com/compute/docs/disks/add-persistent-disk
...
4. Launch a new instance of the portal with the updated container:
  bin/boot_docker -u (sendgrid username) -P (sendgrid password) -e production -p (prod database password) -h portals.broadinstitute.org
