  ...
  def self.up
    Study.all.each do |study|
      ...
      if expression_files.count == 0
        Rails.logger.info "#{Time.now} skipping #{study.name}, no expression data"
        ...
        if cell_array.size > 0
          Rails.logger.info "#{Time.now} processing #{study.name} with 1 expression file (using all_cells arrays)"
          expression_file = study.expression_matrix_files.first
          cell_array.each_slice(DataArray::MAX_ENTRIES).with_index do |slice, index|
            Rails.logger.info "#{Time.now}: Create known cells array ##{index + 1} for #{expression_file.name}:#{expression_file.id} in #{study.name}"
          ...
          Rails.logger.info "#{Time.now} processing #{study.name} with #{expression_files.count} files (in background)"
